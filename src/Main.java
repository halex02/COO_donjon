...
public class Main {
	...
	private static void attaque(Mob source, Mob target) {
		System.out.println(String.format(Local.ACTION_ATTAQUE,
				source.getName(), source.getWeapon().getName()));
	...
	private static void consommable(Mob source, Mob target,
			Consommable consommable) {
		consommable.use(source, target);
		if (player.getStuff().get(consommable) <= 1) {
			player.getStuff().remove(consommable);
		} else {
			player.getStuff().put(consommable,
					player.getStuff().get(consommable) - 1);
		}
		if (target.getPv() > 0) {
			System.out.println(String.format(Local.LAST_PV, target.getPv()));
		} else {
	...
	private static void changeRoom(Room r) {
		...
		room = r;
		room.getMobs().add(player);
		System.out.println(String.format(Local.ACTION_MOVE, player.getName(),
				room.getDescription()));
	...
	private static void move() {
		if (room.getIssues().size() > 1) {
			...
			if (choix > 0 && choix <= room.getIssues().size()) {
				if (room.getMobs().size() > 1) {
					System.out.println(String.format(Local.ACTION_RUN,
							player.getName()));
				...
				if (player.getPv() > 0) {
					changeRoom(room.getIssues().get(
							room.getIssues().keySet().toArray()[choix - 1]));
				}
			} else {
				System.out.println(String.format(Local.IDIOT_ISSUE,
						player.getName()));
			}
		} else {
			if (room.getMobs().size() > 1) {
			...
			if (player.getPv() > 0) {
				...
				System.out.println(" - "
						+ room.getIssues().keySet().toArray()[0]);
				changeRoom(room.getIssues().get(
						room.getIssues().keySet().toArray()[0]));
			}
		}
	...
	private static void inventaire() {
		...
		for (Stuff stuff : player.getStuff().keySet()) {
			...
			if (player.getWeapon().equals(stuff)
					|| player.getArmor().equals(stuff)) {
		...
		for (int mobIndex = 0; mobIndex < room.getMobs().size(); mobIndex++) {
			if (room.getMobs().get(mobIndex).equals(player)
				...
				case 3:// Iventaire
					...
					break;
				case 4:// Danser
					System.out.println(String.format(Local.IDIOT_DANSE,
							player.getName()));
					break;
				case 5:// Attaquer
					if (room.getMobs().size() > 1) {
						...
						if (c > 0 && c <= room.getMobs().size()) {
							...
							System.out.println(String.format(
									Local.IDIOT_ATTAQUE, player.getName()));
						}
					} else {
						System.out.println(String.format(Local.IDIOT_ATTAQUE,
								player.getName()));
					}
	...
	public static void main(String[] args) {
		...
		player = new Mob("Dudule", 100, 15, 2, 2, 2, 2, 0, 1, 3, stuffs,
				weapon, armor, 0, true);
